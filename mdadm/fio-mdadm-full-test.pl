...
system ('./fio-mdadm-test.pl write 1M 8 2G async');
print "Throwaway 1x8G async write test... ";
system ('./fio-mdadm-test.pl write 1M 1 8G async');
print "\n\n";
...
sub loop {
	...
	for (my $loop=0; $loop<3; $loop++) {
		...
		open (FIO, "./fio-mdadm-test.pl $rw $bs $procs $filesize $sync |");
		$rawresults[$loop] = <FIO>;
